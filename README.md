...
**Production-ready** система авторизации партнеров через Telegram Mini App с интеграцией Google Sheets API и современным веб-интерфейсом.

## 🚀 **Live Demo & Deployment**

### 🌐 **Доступные ресурсы:**
- **🎯 Веб-интерфейс**: [https://synthosaicreativestudio-maker.github.io/test/](https://synthosaicreativestudio-maker.github.io/test/)
- **📱 Telegram Mini App**: Авторизация партнеров через веб-интерфейс
...
**🔄 Обновлено 05.09.2025**: Исправлена структура таблицы и терминология

#### 📊 Актуальная структура Google Sheets:
- **Колонка A**: "Код партнера" - уникальный идентификатор
- **Колонка B**: "ФИО партнера" - полное имя
- **Колонка C**: "Телефон партнера" - контактный номер
- **Колонка D**: "Статус авторизации" - автоматически обновляется
- **Колонка E**: "Telegram ID" - ID пользователя в Telegram ✅
- **Колонка F**: "Дата авторизации" - время прохождения ✅

#### 🔧 Исправления:
✅ **Обновлены заголовки колонок** в боте согласно реальной структуре  
✅ **Исправлено соответствие колонок**: E=Telegram ID, F=Дата  
✅ **Обновлена терминология**: сотрудники → партнеры  
✅ **Бот синхронизирован** с реальной структурой таблицы
...
Проект использует Telegram Mini App для удобной авторизации партнеров:

- **Reply клавиатура** с кнопкой запуска Mini App
- **URL конфигурируется** через `WEB_APP_AUTH_URL` в `env`
- **Автоматическая обработка** данных через бот
- **Валидация в реальном времени** кода партнера и телефона
- **Интеграция с MainButton** Telegram для улучшенного UX
- **Google Sheets синхронизация** - автоматическое обновление статуса авторизации
...
### 🤖 **Telegram Bot (Python)**
- **Framework**: aiogram v3.22.0 (Современный async framework)
- **Монолитная архитектура**: Один файл `src/bot.py` с полной функциональностью
- **Google Sheets API**: gspread библиотека с сервисным аккаунтом
- **Авторизация партнеров**: Поиск по коду и телефону
...
```
┌── src/
│   └── bot.py                # Основной бот с полной функциональностью
├── .github/workflows/          # CI/CD конфигурация
├── index.html                  # Telegram Mini App интерфейс
├── credentials.json            # Google Service Account (локально)
