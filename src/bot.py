...
async def find_employee(employee_code: str, phone: str, user_id: int = None) -> Dict[str, Any]:
    ...
    try:
        ...
        for i, record in enumerate(records, start=2):  # start=2 т.к. первая строка - заголовки
            ...
            record_code = str(record.get('Код партнера', '')).strip()
            record_phone = str(record.get('Телефон партнера', '')).strip()
            record_name = str(record.get('ФИО партнера', '')).strip()
            record_status = str(record.get('Статус авторизации', '')).strip()
            ...
            if record_code == employee_code and record_phone == phone:
                ...
                try:
                    worksheet.update_cell(i, 4, 'авторизован')  # Колонка D - Статус авторизации
                    worksheet.update_cell(i, 5, str(user_id))  # Колонка E - Telegram ID
                    worksheet.update_cell(i, 6, datetime.now().strftime('%d.%m.%Y %H:%M'))  # Колонка F - Дата авторизации
                    logger.info(f"📝 Обновлен статус для {record_name}: Telegram ID={user_id}")
        ...
        logger.warning(f"❌ Партнер не найден: код={employee_code}, телефон={phone}")
        return {
            'success': False,
            'message': f'Партнер с кодом {employee_code} и телефоном {phone} не найден в базе данных.\n\nОбратитесь к администратору для добавления в систему.'
        }
...
async def cmd_start(message: types.Message):
    ...
    welcome_text += "🔐 **Система авторизации партнеров**\n\n"
...
async def info_about_system(message: types.Message):
    ...
    info_text = (
        ...
        "Эта система предназначена для авторизации партнеров "
...
async def support_info(message: types.Message):
    ...
    support_text = (
        ...
        "1️⃣ Проверьте правильность ввода кода партнера\n"
        "2️⃣ Убедитесь в корректности номера телефона\n"
        "3️⃣ Обратитесь к администратору системы\n\n"
        "💡 Код партнера должен содержать только цифры\n"
...
async def handle_webapp_data(message: types.Message):
    ...
    try:
        ...
        processing_msg = await message.reply(
            "🔄 **Проверка данных...**\n\n"
            f"👤 Код партнера: `{employee_code}`\n"
            f"📱 Телефон: `{phone}`\n\n"
            "⏳ Поиск в базе данных партнеров...",
            parse_mode="Markdown"
        )
        
        # Поиск сотрудника в Google Sheets
        result = await find_employee(employee_code, phone, user_id)
